...
class Reportes {
		...
		fetch('/reportes/getDetallesFactura', {
			...
			.then(detalles => {
				detalles.map(
					detalle => {
						this.template += `
						<tr id="${detalle.id}" idProducto="${detalle.producto}">
	...
	devolver(detalle,cantidad,producto){
...
document.getElementById("tbl-facturas").addEventListener("click", (e) => {
	...
	if (btn == "btnDetallesFactura") {
		reportes.idfactura = e.target.parentElement.parentElement.getAttribute("id");
...
document.getElementById("table-detalles").addEventListener("click", (e) => {
	...
	if (btn == "btnDevolverDetalle") {
		...
		if (cantidad > 0) {
			...
			producto = e.target.parentElement.parentElement.getAttribute("idProducto");
			reportes.devolver(detalle,cantidad,producto);
		}
	}
})